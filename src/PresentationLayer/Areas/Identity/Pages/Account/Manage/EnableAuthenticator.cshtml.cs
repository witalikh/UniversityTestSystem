...
#nullable disable
    ...
    public class EnableAuthenticatorModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            this._logger.LogInformation("UserEntity with ID '{UserId}' has enabled 2FA with an authenticator app.", userId);
        ...
        private async Task LoadSharedKeyAndQrCodeUriAsync(UserEntity userEntity)
        {
            // Load the authenticator key & QR code URI to display on the form
            var unformattedKey = await this._userManager.GetAuthenticatorKeyAsync(userEntity);
            if (string.IsNullOrEmpty(unformattedKey))
            {
                await this._userManager.ResetAuthenticatorKeyAsync(userEntity);
                unformattedKey = await this._userManager.GetAuthenticatorKeyAsync(userEntity);
            }

            this.SharedKey = this.FormatKey(unformattedKey);

            var email = await this._userManager.GetEmailAsync(userEntity);
